...
{
	//IOutputLog(L"Entering CSerialCommHelper()");
	...
	//IOutputLog(L"Exiting CSerialCommHelper()");
}
...
{
	//IOutputLog(L"Entering CSerialCommHelper::Init()");
	CStringA strTemp(szPortName.c_str());	
	CStringA str;
	str.Format("\\\\.\\%s", strTemp);
	szPortName = str.GetBuffer();
	//szPortName = "\\\\.\\COM14";// For iKnowvations board. If port number is above 9 you need to form a string like this
	...
	{		
		...
		//open the COM Port
		CString strMsg;
		m_hCommPort = ::CreateFileA(szPortName.c_str (),
		...
		//strMsg.Format(L"Opened COM Port %s", szPortName);
		//IOutputLogString(L"COM Port Opened");
		...
		if (! ::SetCommMask(m_hCommPort,EV_RXCHAR|EV_TXEMPTY) )
		{
			...
			strMsg.Format(L"Set Comm Mask Failed ");
...
unsigned __stdcall CSerialCommHelper::ThreadFn(void*pvParam)
{
	...
	while (  abContinue )
	{
		...
		if ( !abRet )
		{
			...
			else if (nError == ERROR_IO_PENDING)
				;
		...
		case WAIT_OBJECT_0:
			{
				_endthreadex(1);
				AfxMessageBox(L"Ending Thread WAIT_OBJECT_0 received");
			}
			break;
		case WAIT_OBJECT_0 + 1:
			{
				...
				ATLTRACE6("RCSerial: Q Unlocked:");
				if ( iAccum > 0 )
				{
					ATLTRACE6("CSerialCommHelper(worker thread):  SetDataReadEvent() len:{%d} data:{%s}",iAccum,(apThis->m_theSerialBuffer.GetData()).c_str ()  );
	...
	if (bError == true)
	{
		...
		pApp->RaiseWeightReadError();
	}
	return 0;
}

void CSerialCommHelper::ClearBuffer()
{
	m_theSerialBuffer.Flush ();
}
...
HRESULT CSerialCommHelper::Write (const char* data,DWORD dwSize)
{
	...
	//	AfxMessageBox("Write Error: IO Pending");
	ATLTRACE6 ( "CSerialCommHelper : Write Error: %d",GetLastError());
	CloseHandle(ov.hEvent);
	std::string szData(data);
	ATLTRACE6("RCSerial:Writing:{%s} len:{%d}",(szData).c_str(),szData.size());
	
	return S_OK;
}


int CSerialCommHelper::getHexVal(char c)
{
	if (c >= '0' && c <= '9')
		return c - '0';
	else if (c >= 'a' && c <= 'f')
...
unsigned char CSerialCommHelper::hexdigit2int(unsigned char xd)
{
	if (xd <= '9')
		return xd - '0';
	xd = tolower(xd);
...
const char * CSerialCommHelper::hexToAscii(char * hexStr)
{
...
HRESULT CSerialCommHelper::Read_Upto (std::string& data,char chTerminator ,long* alCount,long alTimeOut)
{
	...
	{
		ATLTRACE6("CSerialCommHelperUnhandled exception");
		ASSERT ( 0  ) ;
	}
	return hr;
...
HRESULT CSerialCommHelper::Read_N		(std::string& data,long alCount,long  alTimeOut )
{
	...
	{
		ATLTRACE6("CSerialCommHelper Unhandled exception");
		ASSERT ( 0  ) ;
	}
	return hr;
...
HRESULT CSerialCommHelper::ReadAvailable(std::string& data)
{
	...
	{
		ATLTRACE6("CSerialCommHelper Unhandled exception in ReadAvailable()");
		ASSERT ( 0  ) ;
		hr = E_FAIL;
	}
	return hr;

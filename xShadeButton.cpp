...
CxShadeButton::CxShadeButton()
{
	...
	m_flat = m_Checked = m_button_down = m_tracking = m_bDrawBorder = false;
			...
			if ((m_Border)&&((m_tracking)||(m_flat==FALSE))){
				if (lpDrawItemStruct->itemState & ODS_DEFAULT){
					...
					if (m_flat) pDC->Draw3dRect(&r,GetSysColor(COLOR_BTNHILIGHT),GetSysColor(COLOR_BTNSHADOW));
						...
						if(m_bDrawBorder)//drawing of border is flagged by JMC 27.11.17 as not needed by client
							pDC->DrawEdge(&r, EDGE_RAISED, BF_RECT);
				}
			}
//---------------------------------------------------------------------------
	...
	// KPD added the following 23.04.2019
	//hdcMem.DeleteDC();// the destructor will do this for us - No
	//hBitmap.DeleteObject(); //the destructor will do this for us - No
	//pRealDC->DeleteDC();
...
/////////////////////////////////////////////////////////////////////////////
void CxShadeButton::SetBorderDrawMode(bool bFlag)
{ m_bDrawBorder = bFlag;} //set border Draw mode
/////////////////////////////////////////////////////////////////////////////
